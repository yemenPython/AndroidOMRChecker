...
public class Utils {
    ...
    public static boolean checkForMarkers(Mat inputMat, Point[] points, Mat marker) {
//        TODO FULL template matching here
//        TODO Make this run less frequently
        Mat warpLevel1 = four_point_transform(inputMat, points);
        Mat matchOut = new Mat(new Size(inputMat.cols() - marker.cols()+1,inputMat.rows() - marker.rows()+1 ), CvType.CV_32FC1);
        // matchOut will be a float image now!
//        logShape("inputMat",inputMat);
//        logShape("marker",marker);
//        logShape("matchOut",matchOut);
        ...
        Imgproc.rectangle(inputMat, matchLoc, new Point(matchLoc.x + marker.cols(), matchLoc.y + marker.rows()), new Scalar(5, 5, 5), 4);
    ...
    public static Mat four_point_transform(Mat inputMat, Point[] points) {
        ...
        Point[] dst = new Point[] {
                new Point(0, 0),
                new Point(resultWidth, 0),
                new Point(0, resultHeight),
                new Point(resultWidth, resultHeight)
        };
